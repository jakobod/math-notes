...
\textbf{20. Abschluss:} Sei R eine Relation �ber A und sei $\phi$ (reflexiv, usw.) eine Eigenschaft von Relationen.\\
...
%		\textbf{S�tze}. Sei R eine Relation �ber A.
%		\vspace{-0.15cm}
%		\begin{itemize}
...
%		\end{itemize}
%	
%		Der \textbf{Abschluss} einer Relation ist das Hinzuf�gen von einer Eigenschaft (reflexivit�t usw.), durch vereinigen mit den jeweils fehlenden Tupeln.
%		
%		Eine �quivalenzrelation �ber der Menge A erzeugt immer eine Zerlegung von A. Gleichzeitig bestimmt jede Zerlegung von A immer eine �quivalenzrelation. Dies wird am Beispiel der Restklasse Modulo (2) deutlich:\\
%		$[0]$ = \{0,2,4,6,$\dots$\}\\ 
%		$[1]$ = \{1,3,5,6,$\dots$\}\\
%		Dies sind �quivalenzklassen -- Gleichzeitig also auch eine Zerlegung der Menge der nat�rlichen Zahlen $\mathbb{N}$.
%		Diese �quivalenzklassen k�nnen dann wieder vereinigt werden, und man erh�lt die urspr�ngliche Menge--hier also die Menge der nat�rlichen Zahlen $\mathbb{N}$
%		
%		
...
%		\vspace{-0.2cm}
%		Eine Relation ist eine Menge aus Paaren in der Form (a,b) aus einem Kreuzprodukt A $\times$ B. Die Menge R enth�lt nur die Paare, die durch diese Beziehung verkn�pft sind $\rightarrow$ Teilmenge.\\
%		Schreibweisen f�r Relationen sind:\\
%		\textbf{Infix-Notation:} x$\mathcal{R}$y\\
%		\textbf{Pr�fix Notation:} $\mathcal{R}$xy\\
%		\textbf{Postfix-Notation:} xy$\mathcal{R}$\\
%		
%		\vspace{-0.25cm}
%		
%		Beispiele f�r Relationen sind z.B:\\
%		\vspace{-0.4cm}
%		\begin{itemize}
%			\setlength\itemsep{-0.05cm}
...
%		F�r komponierte Relationen gelten die Rechengesetze genauso, wie f�r Mengen. (siehe Rechengesetze VL 2)\\
%		
%		\vspace{-0.25cm}
